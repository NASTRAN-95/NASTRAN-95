...
!
   ...
   COMPLEX , DIMENSION(2) :: Lambda
   COMPLEX , DIMENSION(M,M) :: H
   COMPLEX , DIMENSION(M,M) :: Hl
   COMPLEX , DIMENSION(1) :: Vect
   COMPLEX , DIMENSION(1) :: Mult
   LOGICAL , DIMENSION(1) :: Inth
...
!
         ...
         spag_nextblock_1 = 2
      CASE (2)
         temp = (A(1,1)+A(2,2)+csqrt((A(1,1)+A(2,2))*(A(1,1)+A(2,2))-4.*(A(2,2)*A(1,1)-A(2,1)*A(1,2))))/2.
      ...
      CASE (5)
         ...
         IF ( ard/=0.0 .OR. aid/=0.0 ) THEN
            ...
            IF ( (term1+term2)<=1.0E-9*term3 ) THEN
               spag_nextblock_1 = 6
               CYCLE SPAG_DispatchLoop_1
            ENDIF
         ENDIF
...
!     DETERMINE SHIFT
!
            shift(2) = (A(n-1,n-1)+A(n,n)+csqrt((A(n-1,n-1)+A(n,n))*(A(n-1,n-1)+A(n,n))-4.*(A(n,n)*A(n-1,n-1)-A(n,n-1)*A(n-1,n))))  &
                     & /2.

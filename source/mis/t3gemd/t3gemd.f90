...
!
   ...
   REAL(REAL64) , DIMENSION(9) :: Gb
   REAL(REAL64) , DIMENSION(4) :: Gs
   REAL(REAL64) :: Lx
   REAL(REAL64) :: Ly
   REAL(REAL64) , DIMENSION(3) :: Edglen
   LOGICAL :: Shrflx
   REAL(REAL64) , DIMENSION(18) :: Aic
   REAL(REAL64) :: Jog
   REAL(REAL64) :: Jok
   REAL(REAL64) :: K11
   REAL(REAL64) :: K22
...
!
   REAL(REAL64) , DIMENSION(9) :: aa
   REAL(REAL64) , DIMENSION(3) :: bdum , xx , yy
   REAL(REAL64) :: cosa , cosb , cosc , determ , h1 , h2 , sina , sinb , sinc
...
!
!
!     DOUBLE PRECISION ROUTINE TO SET UP THE REQUIRED SHEAR-RELATED
...
!
      Jok = K11*K22
      IF ( Jok/=0.0D0 ) Jok = 1.0D0/Jok
      Jog = 0.0D0
      IF ( Shrflx ) Jog = Gs(1)*Gs(4) - Gs(2)*Gs(3)
      IF ( Jog/=0.0D0 ) Jog = 1.0D0/Jog
   ENDIF
END SUBROUTINE t3gemd

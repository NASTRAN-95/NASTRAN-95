!*==sort.f90  processed by SPAG 7.61RG at 01:00 on 21 Mar 2022
...
!
      ...
      IF ( zi/=zn ) GOTO 600
      IF ( isort==1 ) GOTO 700
      IF ( Z(key+1,i)<=Z(key+1,n) ) GOTO 700
      GOTO 600
   ENDIF
!
...
!
 ...
 500  IF ( .NOT.rvsbcd ) THEN
   ...
   ELSE
      ...
      IF ( khrfn1(zero,4,zi,2)/=khrfn1(zero,4,zn,2) ) GOTO 600
      IF ( khrfn1(zero,4,zi,3)<khrfn1(zero,4,zn,3) ) GOTO 700
      IF ( khrfn1(zero,4,zi,3)/=khrfn1(zero,4,zn,3) ) GOTO 600

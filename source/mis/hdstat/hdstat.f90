...
!
   REAL :: amax , amaxx , amaxy , amin , aminx , aminy , d , dx , ei , exx , r , s , s1 , t , tx , ty , ve , xi , xii , ye , yi ,   &
         & yii , ze , zi , zii
...
!
               DO j = 1 , ib , 5
                  ...
                  IF ( (abs(t)<exx) .AND. (s*s1<=0.) ) CYCLE SPAG_Loop_3_1
                  ...
                  IF ( r/=0. ) THEN
                     ...
                     IF ( t>=Xa(jx) ) THEN
                        ...
                        IF ( (t==Ccc(j+3+jc)) .OR. (t==Ccc(j+4+jc)) ) CYCLE SPAG_Loop_2_2
                        s = t - Ccc(j+3+jc)
                        s1 = t - Ccc(j+4+jc)
                        IF ( s*s1<=0. ) i = i + 1
                     ENDIF
                  ENDIF
               ENDDO
               IF ( mod(i,2)==0 ) EXIT SPAG_Loop_2_2
            ENDDO SPAG_Loop_3_1
            nx = nx + 1
            Xa(nx) = Xa(jx)
            Ya(nx) = Ya(jx)

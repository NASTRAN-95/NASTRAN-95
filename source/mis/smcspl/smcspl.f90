!*==smcspl.f90 processed by SPAG 8.01RF 16:19  2 Dec 2023
...
!
         ...
         CALL read(*20,*40,iscr1,Zi(ispill),4,0,4)
         mm2 = Zi(ispill+1)
         mterms = Zi(ispill+3)
         mwords = mm2 + mterms*ivwrds
!      PRINT *,' SMCSPL,MM2,MTERMS,MWORDS=',MM2,MTERMS,MWORDS
         CALL read(*20,*40,iscr1,Zi(ispill+4),mwords,1,mwords)
!
...
!      PRINT *,' SMCSPL,MCOL,KCOL=',MCOL,KCOL
         SPAG_Loop_1_2: DO WHILE ( Mcol==kcol )
!
...
!
                     IF ( index1==0 ) EXIT SPAG_Loop_2_1
                     indext = memfre
                  ELSE
!
...
!      PRINT *,' SMCSPL,A1200,MWORDS,ZI(INDEX1+2=',MWORDS,ZI(INDEX1+2)
                        IF ( Zi(index2+2)<(mwords+4) ) CYCLE SPAG_Loop_1_2
                        iidx = index2
                        spag_nextblock_1 = 2
                        CYCLE SPAG_DispatchLoop_1
                     ELSE
...
!      PRINT *,' SMCSPL,B1800,IMIDX1,ISPACE1=',IMIDX1,ISPACE1
                     ...
                     SPAG_Loop_3_3: DO j = memcoln , 1 , -1
                        IF ( j/=kcol ) THEN
                           IF ( j/=i ) THEN
                              jdir = (j-1)*4 + 1
                              IF ( Zi(jdir)/=0 ) THEN
...
!     &,         ISPACE1,ISPACE2,ITOTAL,MWORDS
                                 IF ( itotal<(mwords+4) ) EXIT SPAG_Loop_3_3
!

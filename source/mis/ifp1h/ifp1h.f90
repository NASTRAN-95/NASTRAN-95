...
!
   REAL , DIMENSION(2,3) , SAVE :: bcd
   REAL , DIMENSION(4) :: cc
   REAL , SAVE :: comma
   INTEGER , DIMENSION(2,60) :: comp
   INTEGER , DIMENSION(2,19) , SAVE :: comp1 , comp2 , comp3
   INTEGER , DIMENSION(2,3) , SAVE :: comp4
   INTEGER , DIMENSION(1) :: core
   INTEGER , DIMENSION(401) :: corey
   LOGICAL , SAVE :: debug
   INTEGER , SAVE :: equal , llc , lll , mzero , ncomp , seti
   INTEGER :: err , force , i , icomp , idupl , ie , ielem , iend , ieq , ii , iisub , inc , irept , iscan , iset , iwds , iword ,  &
            & j , jc , jcomp , jj , jx , k , kc , kk , lline , max , min , mome , norm , nrp , nscan , nsv , nwdsf , nwdss , stress
   INTEGER , DIMENSION(90) , SAVE :: etab
   REAL :: flag , shea , xblank
   INTEGER , DIMENSION(400) :: icse
   INTEGER , DIMENSION(10) :: isp
   INTEGER , DIMENSION(3) :: keywds
   INTEGER , DIMENSION(4) :: ll
   INTEGER , DIMENSION(2) , SAVE :: nam
   REAL , DIMENSION(1) :: rcore
   INTEGER , DIMENSION(5) :: save
   INTEGER , DIMENSION(30) , SAVE :: sp
   INTEGER , DIMENSION(10,17) :: tab
   INTEGER , DIMENSION(10,9) , SAVE :: tab1
   INTEGER , DIMENSION(10,8) , SAVE :: tab2
   EXTERNAL ifp1d , khrfn1 , khrfn2 , khrfn3 , korsz , mesage , page , page1 , page2 , read , sswtch , xrcard
!
...
!
                           ...
                           IF ( ii/=4 .AND. ii/=16 ) THEN
                              ...
                              ENDIF
                              IF ( jj==11 .OR. (jj>=16 .AND. jj<=21) ) THEN
                                 spag_nextblock_2 = 2
                                 CYCLE SPAG_DispatchLoop_2
                              ENDIF
                           ENDIF
...
!
               ...
               ENDIF
               err = -643
               IF ( core(iscan)/=10000000 .AND. core(iscan)/=20000000 ) THEN
                  spag_nextblock_1 = 2
                  CYCLE SPAG_DispatchLoop_1
               ENDIF
...
!
                  ...
                  core(icomp+max) = kk
                  err = -641
                  IF ( max==2 .AND. rcore(icomp+2)>rcore(icomp+1) ) THEN
                     spag_nextblock_1 = 2
                     CYCLE SPAG_DispatchLoop_1
                  ENDIF
               ELSEIF ( jj==0 ) THEN

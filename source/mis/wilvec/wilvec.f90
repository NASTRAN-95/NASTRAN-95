!*==wilvec.f90 processed by SPAG 8.01RF 16:18  2 Dec 2023
...
!
   ...
   REAL(REAL64) , DIMENSION(1) :: F
   REAL(REAL64) , DIMENSION(1) :: P
   REAL(REAL64) , DIMENSION(1) :: Q
   REAL(REAL64) , DIMENSION(1) :: R
   REAL(REAL64) , DIMENSION(Nx,1) :: Vec
   INTEGER , DIMENSION(1) :: Svec
!
! Local variable declarations rewritten by SPAG
!
   REAL(REAL64) :: deps , rmult , rrmult , sft , sftinv , value , vmult , w , x , y , z
...
!
         ...
         spag_nextblock_1 = 2
      CASE (2)
         IF ( dabs(Val(i))+dabs(Val(i+1))>=rmult ) THEN
            IF ( Val(i)==zero ) THEN
               spag_nextblock_1 = 5
               CYCLE SPAG_DispatchLoop_1
            ENDIF
            IF ( dabs(one-Val(i)/Val(i+1))>rrmult ) THEN
               spag_nextblock_1 = 4
               CYCLE SPAG_DispatchLoop_1
            ENDIF
         ENDIF
...
!
         ...
         spag_nextblock_1 = 8
      CASE (7)
         IF ( dabs(Val(v2))+dabs(Val(v2+1))>=rmult ) THEN
            IF ( dabs(one-Val(v2)/Val(v2+1))>rmult ) THEN
               spag_nextblock_1 = 8
               CYCLE SPAG_DispatchLoop_1
            ENDIF
         ENDIF
...
!
                     y = dabs(V(max))
                     IF ( sngl(w)==sngl(y) ) EXIT SPAG_Loop_2_1
                     IF ( iter/=maxitr ) THEN
                        w = y
!
!     PIVOT V.
...
!
                     IF ( dabs(Val(vector+1))+dabs(Val(vector))>=rmult ) THEN
                        IF ( dabs(Val(vector+1)-Val(vector))>rmult*dabs(Val(vector+1)) ) THEN
                           spag_nextblock_2 = 5
                           CYCLE SPAG_DispatchLoop_2
                        ENDIF
                     ENDIF

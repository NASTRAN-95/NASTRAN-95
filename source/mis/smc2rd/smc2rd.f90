!*==smc2rd.f90 processed by SPAG 8.01RF 16:19  2 Dec 2023
!!SPAG Open source Personal, Educational or Academic User  NON-COMMERCIAL USE - Not for use on proprietary or closed source code
!!SPAG Open source Personal, Educational or Academic User  NON-COMMERCIAL USE - Not for use on proprietary or closed source code
 
 
SUBROUTINE smc2rd(Zi,Zd,Zil,Zol,Nar,Lasrow,Dtemp,I1,I2,I3)
   USE i_smcomx
   USE I_SMCOMX
   IMPLICIT NONE
   INCLUDE 'SMCOMX.COM'
   INTEGER I1 , I2 , I3
   DOUBLE PRECISION Dtemp(I3) , Zd(10) , Zil(I1,I2) , Zol(I2,2)
   ...
   DO k = 1 , ncol
      ...
      SPAG_DispatchLoop_1: DO
         ...
         CASE (1)
            ...
            IF ( kk==0 ) kk = I2
            Lasrow(kk) = 0
!      PRINT *,' SMC2RD PROCESSING COLUMN K=',K
            kcol = k
            kdir = k*4 - 3
            kmidx = Zi(kdir)
!
...
!      PRINT *,' CALLING SMCSPL FOR K=',K
            ...
            ENDDO
!      PRINT *,' SMC2RD,K,KFRCOL,KLSCOL,KROW1,KROWN,KAROWS='
!      PRINT *,         K,KFRCOL,KLSCOL,KROW1,KROWN,KAROWS
!
...
! SET IILROW TO FIRST ROW POSITION FOR NEW COLUMN DATA.
!
                              kfrcolg = klscolp + 1
                              iilrow = iilrow1
                              spag_nextblock_1 = 2
...
!
                        jvidx = jvidx/2 + 1
!      PRINT *,' DIAGONAL TERM,JCOL=',JCOL,ZD(JVIDX)
...
!
                  ...
                  IF ( kfrcol/=k ) THEN
                     IF ( jlim2<jlim1 ) THEN
                        ...
                        jlim2 = I2
                     ENDIF
!      PRINT *,' K,ILIM1,ILIM2,JLIM1,JLIM2,JLIM4,IILROW1,NROWS'
!      PRINT *,  K,ILIM1,ILIM2,JLIM1,JLIM2,JLIM4,IILROW1,NROWS
                     IF ( k/=1 ) THEN
!
...
!
!      CALL AUDIT ( 'COMP-ZOL', 1 )
                        DO kbc = jlim1 , jlim2
                           Zol(kbc,ic1) = Zil(iilrow1,kbc)*Zol(kbc,ic2)
!      IF ( K.EQ.16) PRINT *,' KBC,IC1,ZOL-1=',KBC,IC1,ZOL(KBC,IC1)
!      IF ( K.EQ.16) PRINT *,' ZIL,ZOL=',ZIL(IILROW1,KBC),ZOL(KBC,IC2)
!      IF ( K.EQ.16) PRINT *,' KBC,IC1,IILROW1,IC2=',KBC,IC1,IILROW1,IC2
                        ENDDO
                        IF ( jlim4/=0 ) THEN
                           DO kbc = 1 , jlim4
                              Zol(kbc,ic1) = Zil(iilrow1,kbc)*Zol(kbc,ic2)
!      IF ( K.EQ.16 ) PRINT *,' KBC,IC1,ZOL-2=',KBC,IC1,ZOL(KBC,IC1)
!      IF ( K.EQ.16) PRINT *,' ZIL,ZOL=',ZIL(IILROW1,KBC),ZOL(KBC,IC2)
!      IF ( K.EQ.16) PRINT *,' KBC,IC1,IILROW1,IC2=',KBC,IC1,IILROW1,IC2
                           ENDDO
                        ENDIF
                     ENDIF
!      CALL AUDIT ( 'COMP-ZOL', 2 )
!      IF ( K .EQ.16 )
!     & CALL KBHELPRD( KFRCOL, KLSCOL, ZOL, ZIL, I1, I2, LASROW )
!      CALL AUDIT ( 'COMP-ZIL', 1 )
                     DO i = iilrow1 , ilim1
                        Dtemp(i) = 0.0D0
                     ENDDO
!
...
!
                           zoltmp = Zol(j,ic1)
                           CALL smccrd(Dtemp(iilrow1),Zil(iilrow1,j),limit-iilrow1+1,zoltmp)
                        ENDIF
!      DO 4020 I = IILROW1, LIMIT
!      DTEMP(I) = DTEMP(I) + ZIL( I, J ) * ZOLTMP
!      IF ( K .EQ. 16 ) PRINT *,' 1-I,J,DTEMP,ZIL,ZOLTMP='
!      IF ( K .EQ. 16 ) PRINT *,    I,J,DTEMP(I),ZIL(I,J),ZOLTMP
!4020  CONTINUE
                     ENDDO
                     IF ( jlim4/=0 ) THEN
!
...
!      DTEMP(I) = DTEMP(I) + ZIL( I, J ) * ZOLTMP
!      IF ( K .EQ.16 ) PRINT *,' 2-I,J,DTEMP,ZIL,ZOLTMP='
!      IF ( K .EQ.16 ) PRINT *,    I,J,DTEMP(I),ZIL(I,J),ZOLTMP
!4023  CONTINUE
                        ENDDO
                     ENDIF
...
!      DO 4040 I = 1, LIMIT
!      DTEMP(I) = DTEMP(I) + ZIL( I, J ) * ZOLTMP
!      IF ( K .EQ.16 ) PRINT *,' 3-I,J,DTEMP,ZIL,ZOLTMP='
!      IF ( K .EQ.16 ) PRINT *,    I,J,DTEMP(I),ZIL(I,J),ZOLTMP
!4040  CONTINUE
                        ENDDO
                        IF ( jlim4/=0 ) THEN
...
!      DO 4043 I = 1, LIMIT
!      DTEMP(I) = DTEMP(I) + ZIL( I, J ) * ZOLTMP
!      IF ( K .EQ.16 ) PRINT *,' 4-I,J,DTEMP,ZIL,ZOLTMP='
!      IF ( K .EQ.16 ) PRINT *,    I,J,DTEMP(I),ZIL(I,J),ZOLTMP
!4043  CONTINUE
                           ENDDO
                        ENDIF
                     ENDIF
!      CALL AUDIT ( 'COMP-ZIL', 2 )
!
! UPDATE EACH ACTIVE ROW TERM FOR COLUMN "K" BY SUBTRACTING "DTEMP"
!
!      CALL AUDIT ( 'UPDATECO', 1 )
                     DO i = iilrow1 , ilim1

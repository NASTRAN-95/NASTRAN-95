!*==mma322.f90  processed by SPAG 7.61RG at 01:00 on 21 Mar 2022
...
!
!
!     MMA322 PERFORMS THE MATRIX OPERATION IN REAL DOUBLE PRECISION
!       (+/-)A(T & NT) * B (+/-)C = D
!
!     MMA322 USES METHOD 32 WHICH IS AS FOLLOWS:
...
!      PRINT *,' IROW1,N=',IROW1,IROWN
                        DO k = irow1 , irown
!      PRINT *,' K,D,A,B=',K,DTEMP,ZD(INDXAV+K),ZD(INDXBV+K)
...
!      PRINT *,' ADDING C,DD,C=',DD(1),ZD(INDXCV)
                     dd(1) = dd(1) + Zd(indxcv)
                     EXIT
                  ENDIF
               ENDDO

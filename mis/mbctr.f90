...
!     DOUBLE CORNER BOXES
!
            IF ( yl<=y(2) .AND. yr>=y(2) .AND. xt<x(2) .AND. xb>=x(2) .AND. yl<=y(1) .AND. yr>=y(1) .AND. xt<x(1) .AND. xb>=x(1) )  &
               & THEN
!
!     LH CORNERS
!
               IF ( yl<=yth .AND. yr>=yth .AND. yl<=ybt .AND. yr>=ybt ) pa = 0.5*((2.0*yr-yth-y(2))*(x(2)-xt)+(2.0*yr-y(2)-y(1))    &
                  & *(x(1)-x(2))+(2.0*yr-y(1)-ybt)*(xb-x(1)))/Boxa
               IF ( xt<xrh .AND. xb>=xrh .AND. yl<=ybt .AND. yr>=ybt ) pa = 0.5*((x(2)-xrh)*(yr-y(2))+(2.0*yr-y(2)-y(1))*(x(1)-x(2))&
                  & +(2.0*yr-y(1)-ybt)*(xb-x(1)))/Boxa
               IF ( yl<=yth .AND. yr>=yth .AND. xt<xrt .AND. xb>=xrt ) pa = 0.5*((2.0*yr-yth-y(2))*(x(2)-xt)+(2.0*yr-y(2)-y(1))     &
                  & *(x(1)-x(2))+(xrt-x(1))*(yr-y(1)))/Boxa
               IF ( xt<xrh .AND. xb>=xrh .AND. xt<xrt .AND. xb>=xrt ) pa = 0.5*((x(2)-xrh)*(yr-y(2))+(2.0*yr-y(2)-y(1))*(x(1)-x(2)) &
                  & +(xrt-x(1))*(yr-y(1)))/Boxa
               IF ( i==1 ) GOTO 20
!
            ELSEIF ( yl<y(3) .AND. yr>=y(3) .AND. xt<x(3) .AND. xb>=x(3) .AND. yl<y(4) .AND. yr>=y(4) .AND. xt<x(4) .AND. xb>=x(4) )&
                   & THEN
!
!     RH CORNERS
!
               IF ( yl<yth .AND. yr>=yth .AND. yl<ybt .AND. yr>=ybt ) pa = 0.5*((yth+y(3)-2.0*yl)*(x(3)-xt)+(y(3)+y(4)-2.0*yl)      &
                  & *(x(4)-x(3))+(y(4)+ybt-2.0*yl)*(xb-x(4)))/Boxa
               IF ( xt<xlh .AND. xb>=xlh .AND. yl<ybt .AND. yr>=ybt ) pa = 0.5*((x(3)-xlh)*(y(3)-yl)+(y(3)+y(4)-2.0*yl)*(x(4)-x(3)) &
                  & +(y(4)+ybt-2.0*yl)*(xb-x(4)))/Boxa
               IF ( yl<yth .AND. yr>=yth .AND. xt<xlt .AND. xb>=xlt ) pa = 0.5*((yth+y(3)-2.0*yl)*(x(3)-xt)+(y(3)+y(4)-2.0*yl)      &
                  & *(x(4)-x(3))+(xlt-x(4))*(y(4)-yl))/Boxa
               IF ( xt<xlh .AND. xb>=xlh .AND. xt<xlt .AND. xb>=xlt ) pa = 0.5*((x(3)-xlh)*(y(3)-yl)+(y(3)+y(4)-2.0*yl)*(x(4)-x(3)) &
                  & +(xlt-x(4))*(y(4)-yl))/Boxa
!
!     SINGLE CORNER BOXES
!
...
!     FWD LH CORNER
!
               IF ( yl<=ybl .AND. yr>=ybl .AND. xt<xrh .AND. xb>=xrh ) pa = .5*((y(2)-ybl)*(xb-x(2))+(2.*xb-x(2)-xrh)*(yr-y(2)))    &
                  & /Boxa
               IF ( xt<xll .AND. xb>=xll .AND. xt<xrh .AND. xb>=xrh ) pa = .5*((2.*xb-xll-x(2))*(y(2)-yl)+(2.*xb-xrh-x(2))*(yr-y(2))&
                  & )/Boxa
               IF ( xt<xll .AND. xb>=xll .AND. yl<ybh .AND. yr>=ybh ) pa = .5*((2.*xb-x(2)-xll)*(y(2)-yl)+(ybh-y(2))*(xb-x(2)))/Boxa
...
!
               IF ( x(1)<x(4) ) THEN
                  IF ( yl<ybt .AND. yr>=ybt .AND. yl<=ytl .AND. yr>=ytl ) pa = .5*((2.*yr-ytl-y(1))*(x(1)-xt)+(2.*yr-y(1)-ybt)      &
                     & *(xb-x(1)))/Boxa
               ENDIF
               IF ( yl<=ytl .AND. yr>=ytl .AND. xt<xrt .AND. xb>=xrt ) pa = .5*((y(1)-ytl)*(x(1)-xt)+(x(1)+xrt-2.*xt)*(yr-y(1)))    &
                  & /Boxa
               ...
               IF ( yl<ybt .AND. yr>=ybt .AND. xt<xrl .AND. xb>=xrl ) pa = .5*((x(1)-xrl)*(yr-y(1))+(2.*yr-y(1)-ybt)*(xb-x(1)))/Boxa
               IF ( i==1 ) GOTO 20
            ELSEIF ( yl<y(3) .AND. yr>=y(3) .AND. xt<x(3) .AND. xb>=x(3) ) THEN
!
!     FWD RH CORNER
!
               IF ( yr>=ybr .AND. yl<ybr .AND. xt<xlh .AND. xb>=xlh ) pa = 0.5*((x(3)-xlh)*(y(3)-yl)+(y(3)+ybr-2.0*yl)*(xb-x(3)))   &
                  & /Boxa
               IF ( xt<xlh .AND. xb>=xlh .AND. xt<xlr .AND. xb>=xlr ) pa = 0.5*(xlr-xlh)*(y(3)-yl)/Boxa
               IF ( yr>=yth .AND. yl<yth .AND. xt<xlr .AND. xb>=xlr ) pa = .5*((yth+y(3)-2.*yl)*(x(3)-xt)+(y(3)-yl)*(xlr-x(3)))/Boxa
               IF ( yr>=yth .AND. yl<yth .AND. yr>=ybr .AND. yl<ybr ) pa = 0.5*((yth+y(3)-2.0*yl)*(x(3)-xt)+(y(3)+ybr-2.0*yl)       &
                  & *(xb-x(3)))/Boxa
...
!
               IF ( x(4)<x(1) ) THEN
                  IF ( yr>=ytr .AND. yl<ytr .AND. yl<ybt .AND. yr>=ybt ) pa = 0.5*((ytr+y(4)-2.0*yl)*(x(4)-xt)+(y(4)+ybt-2.0*yl)    &
                     & *(xb-x(4)))/Boxa
               ENDIF
               IF ( yr>=ytr .AND. yl<ytr .AND. xt<xlt .AND. xb>=xlt ) pa = .5*((xlt-x(4))*(y(4)-yl)+(ytr+y(4)-2.*yl)*(x(4)-xt))/Boxa
               IF ( yl<ytt .AND. yr>=ytt .AND. yr>=ytr .AND. yl<ytr ) pa = 0.5*(ytr-ytt)*(x(4)-xt)/Boxa
               IF ( xt<xlt .AND. xb>=xlt .AND. xt<xrr .AND. xb>=xrr ) pa = 0.5*((xlt+x(4)-2.0*xt)*(y(4)-yl)+(x(4)+xrr-2.0*xt)       &
                  & *(yr-y(4)))/Boxa
               IF ( yl<ytt .AND. yr>=ytt .AND. xt<xrr .AND. xb>=xrr ) pa = .5*((y(4)-ytt)*(x(4)-xt)+(x(4)+xrr-2.*xt)*(yr-y(4)))/Boxa
!
!     HINGE + T. E. BOXES
!
...
!
               IF ( yr>=ybr .AND. yl<ybr .AND. xt<xrr .AND. xb>=xrr .AND. xt<xrh .AND. xb>=xrh .AND. yl<yth .AND. yr>=yth )         &
                  & pa = 1.0 - 0.5*((yr-yth)*(xrh-xt)+(xb-xrr)*(yr-ybr))/Boxa
               IF ( yr>=ybr .AND. yl<ybr .AND. xt<xrr .AND. xb>=xrr .AND. xt<xrh .AND. xb>=xrh .AND. xt<xlh .AND. xb>=xlh )         &
                  & pa = 0.5*((2.0*xb-xlh-xrh)*Boxw-(yr-ybr)*(xb-xrr))/Boxa
               IF ( yl<yth .AND. yr>=yth .AND. xt<xrh .AND. xb>=xrh .AND. xt<xrr .AND. xb>=xrr .AND. xt<xlr .AND. xb>=xlr )         &
                  & pa = 0.5*((xlr+xrr-2.0*xt)*Boxw-(yr-yth)*(xrh-xt))/Boxa
               IF ( xt<xlh .AND. xb>=xlh .AND. xt<xlr .AND. xb>=xlr .AND. xt<xrh .AND. xb>=xrh .AND. xt<xrr .AND. xb>=xrr )         &
...
!
               ...
               ENDIF
!
               IF ( yl<=ytl .AND. yr>=ytl .AND. yl<yth .AND. yr>=yth .AND. xt<xll .AND. xb>=xll .AND. xt<xrh .AND. xb>=xrh )        &
                  & pa = 1.0 - 0.5*((xll-xt)*(ytl-yl)+(yr-yth)*(xrh-xt))/Boxa
               IF ( yl<=ybl .AND. yr>=ybl .AND. yl<=ytl .AND. yr>=ytl .AND. yl<yth .AND. yr>=yth .AND. xt<xrh .AND. xb>=xrh )       &
                  & pa = 1.0 - 0.5*((ytl+ybl-2.0*yl)*Boxl+(yr-yth)*(xrh-xt))/Boxa
               IF ( yl<ybh .AND. yr>=ybh .AND. xt<xll .AND. xb>=xll .AND. yl<=ytl .AND. yr>=ytl .AND. yl<yth .AND. yr>=yth )        &
                  & pa = 1.0 - 0.5*((ytl-yl)*(xll-xt)+(2.0*yr-yth-ybh)*Boxl)/Boxa
               IF ( yl<=ytl .AND. yr>=ytl .AND. yl<yth .AND. yr>=yth .AND. yl<=ybl .AND. yr>=ybl .AND. yl<ybh .AND. yr>=ybh )       &
                  & pa = 0.5*(yth+ybh-ytl-ybl)/Boxw
               IF ( yl<=ytl .AND. yr>=ytl .AND. yl<=ybl .AND. yr>=ybl .AND. yl<ybt .AND. yr>=ybt .AND. xt<xrt .AND. xb>=xrt )       &
                  & pa = 0.5*((2.0*yr-ytl-ybl)*Boxl-(yr-ybt)*(xb-xrt))/Boxa
               IF ( yl<ytl .AND. yr>=ytl .AND. xt<xll .AND. xb>=xll .AND. xt<xlt .AND. xb>=xlt .AND. yl<ybt .AND. yr>=ybt )         &
                  & pa = 1.0 - 0.5*((ytl-yl)*(xll-xt)+(xb-xlt)*(ybt-yl))/Boxa
               IF ( xt<xll .AND. xb>=xll .AND. xt<xrl .AND. xb>=xrl .AND. yl<ybt .AND. yr>=ybt .AND. xt<xlt .AND. xb>=xlt )         &
                  & pa = 0.5*((2.0*xb-xll-xrl)*Boxw-(xb-xlt)*(ybt-yl))/Boxa
               IF ( yl<ytl .AND. yr>=ytl .AND. xt<xll .AND. xb>=xll .AND. xt<xlt .AND. xb>=xlt .AND. xt<xrt .AND. xb>=xrt )         &
                  & pa = 0.5*((xlt+xrt-2.0*xb)*Boxw-(ytl-yl)*(xll-xt))/Boxa
               IF ( xt<xll .AND. xb>=xll .AND. xt<xlt .AND. xb>=xlt .AND. xt<xrl .AND. xb>=xrl .AND. xt<xrt .AND. xb>=xrt )         &
...
!
               IF ( xt<xlt .AND. xb>=xlt .AND. yr>=ybr .AND. yl<ybr .AND. yl<ybt .AND. yr>=ybt .AND. xt<xrr .AND. xb>=xrr )         &
                  & pa = 1.0 - 0.5*((xb-xlt)*(ybt-yl)+(yr-ybr)*(xb-xrr))/Boxa
               IF ( yl<ytt .AND. yr>=ytt .AND. yl<ybt .AND. yr>=ybt .AND. yr>=ybr .AND. yl<ybr .AND. xt<xrr .AND. xb>=xrr )         &
                  & pa = 0.5*((2.0*yr-ytt-ybt)*Boxl-(yr-ybr)*(xb-xrr))/Boxa
               IF ( yr>=ytr .AND. yl<ytr .AND. yr>=ybr .AND. yl<ybr .AND. xt<xlt .AND. xb>=xlt .AND. yl<ybt .AND. yr>=ybt )         &
                  & pa = 0.5*((ytr+ybr-2.0*yl)*Boxl-(xb-xlt)*(ybt-yl))/Boxa
               IF ( yl<ytt .AND. yr>=ytt .AND. yr>=ytr .AND. yl<ytr .AND. yl<ybt .AND. yr>=ybt .AND. yr>=ybr .AND. yl<ybr )         &

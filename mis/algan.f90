...
SUBROUTINE algan
   ...
   REAL Alpha(21,10) , B1(21) , B2(21) , Bs(21) , Chordd(21) , Cord(21) , Delx(21) , Dely(21) , Epz(80,4) , Pp(21) , Qq(21) ,       &
      ...
      & Xsemj(21,31) , Xsta(21,10) , Xtemp(100) , Ycamb(21,10) , Yprime(100) , Ysemi(21,31) , Ysemj(21,31) , Zcamb(21,10) , Zout(21)&
      & , Zp(21,70) , Zr(21) , Zsemi(21,31) , Zsemj(21,31) , Zz(21)
   INTEGER Ifangs(10) , Ipgeom , Iprtc , Istrml , Kpts(21) , Ksystm(90) , Log1 , Log2 , Log3 , Log4 , Log5 , Log6 , Lpunch , Naero ,&
         & Nanal , Narbit , Nbldes
   COMMON /contrl/ Nanal , Naero , Narbit , Log1 , Log2 , Log3 , Log4 , Log5 , Log6
   COMMON /system/ Ksystm , Lpunch
   COMMON /ud3anc/ Epz , R , Zout , Ss , X , Yprime , Ysemi , Xsemi , Zp , Zsemi , Title2 , Xhere , Xtemp , Rad , Temp1 , Temp2 ,   &
                 & Temp3 , Temp4 , Zr , B1 , B2 , Pp , Qq , Zz , Rle , Tc , Te , Cord , Delx , Dely , S , Bs , Xsemj , Ysemj ,      &
   ...
   REAL alp , alpb(10,21) , alphb , ang , area , ax , beta1 , beta2 , blafor(10,21) , block(10,21) , bx , c1 , ccord(21) , chd ,    &
      ...
      & t , t1 , t2 , tav3 , tcs(21) , tes(21) , tharr(21,10) , thck , torcon , tq(21) , ts , u , vol , w1 , w2 , x1 , x2 , xb ,    &
      & xcamb(21,10) , xd , xdel , xdum , xint , xj , xmn , xp(21,70) , xplot , xs(21,70) , xsign , xz , y1 , y2
   INTEGER i , idata(24) , idum , ifcord , ifplot , igrd1 , igrd2 , igrd3 , ii , iidum , ijdum , ikdum , inast , iprint , ipunch ,  &
         & irle , irt , irte , isbs , isecn , isplit , istak , j , jd , jloop , k , kpt , lnct , n , n1 , n2 , nblade , nd1 , nd2 , &
...
99009 FORMAT (10X,F7.2,3X,F8.3,F10.3,3F10.4)
   ...
   DO j = 1 , nlines
      ...
      CALL alg13(j,ys,yp,xs,xp,Ysemi,Xsemi,Log1,Log2,npoint,iprint,beta1,beta2,p,q,yzero,t,yone,xdel,ydel,z,chd,lnct,ifcord,sq,sb,  &
               & isecn,Xsemj,Ysemj,istak,Xhere,X,Ss,nstns,R,Xtemp,Yprime,Rad,Epz,bx,Sigma,ccord,isplit,yzeros,ts,yones,zspmxt,      &
               & perspj,inast,irle,irte,tharr)
      CALL alg15(X,Ss,100,stackx,bx,1,1)
      DO i = 1 , 100
         X(i) = X(i) - stackx
         Ss(i) = Ss(i) - bx
...
99010 FORMAT (/10X,38HCARTESIAN COORDINATES ON STREAMSURFACE,I3,//10X,8HPOINT NO,5X,2HZ1,12X,2HX1,12X,2HY1,16X,2HZ2,12X,2HX2,12X,   &
             &2HY2,/)
      i = 1
      DO
...
99012       FORMAT (1H1,9X,8HPOINT NO,5X,2HZ1,12X,2HX1,12X,2HY1,16X,2HZ2,12X,2HX2,12X,2HY2,/)
         ...
         ENDIF
      ENDDO
 50   IF ( Iprtc==1 ) WRITE (Log2,99013) i , Zsemi(j,i) , Xsemi(j,i) , Ysemi(j,i)
99013 FORMAT (10X,I5,3X,1P,3E14.5)
      GOTO 150
 100  IF ( Iprtc==1 ) WRITE (Log2,99014) i , Zsemi(j,i) , Xsemi(j,i) , Ysemi(j,i) , Zsemj(j,i) , Xsemj(j,i) , Ysemj(j,i)
99014 FORMAT (10X,I5,3X,1P,3E14.5,4X,1P,3E14.5)
 150  i = i + 1
      lnct = lnct + 1
      IF ( i<=31 ) THEN
         IF ( lnct<=59 .AND. isecn==2 ) GOTO 100
         IF ( isecn/=2 ) THEN
            IF ( lnct>59 ) THEN
               IF ( Iprtc/=0 ) WRITE (Log2,99034)
               IF ( Iprtc==1 ) WRITE (Log2,99039)

...
!
   ...
   iclos = 0
   i = Nrigid + 1
 100  IF ( abs(Val(i))+abs(Val(i+1))>=rmult ) THEN
      IF ( Val(i)==zero ) GOTO 400
      IF ( abs(one-Val(i)/Val(i+1))>rrmult ) GOTO 300
   ENDIF
   IF ( iclos==0 ) iclos = i
   GOTO 400
 200  DO i1 = iclos , i
...
!
   ...
   ENDIF
   GOTO 700
 600  IF ( abs(Val(V2))+abs(Val(V2+1))>=rmult ) THEN
      IF ( abs(one-Val(V2)/Val(V2+1))>rmult ) GOTO 700
   ENDIF
   l1 = Vloc(V2)
   l2 = Vloc(V2+1)
   n1 = min0(l1,l2)
...
!
         IF ( abs(Val(vector+1))+abs(Val(vector))>=rmult ) THEN
            IF ( abs(Val(vector+1)-Val(vector))>rmult*abs(Val(vector+1)) ) GOTO 900
         ENDIF
         l1 = Vloc(vector+1)

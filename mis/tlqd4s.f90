...
SUBROUTINE tlqd4s
   ...
   REAL aa , abbd(6,6) , alfab(3) , alfam(3) , alpha(6) , alphad(6) , alphae(3) , alphal(3) , alpham(6) , bb , bdum , bgpdm(3,4) ,  &
      ...
      & , g2(9) , g2i(9) , galpha(3) , gbar(9) , gdum , gepsbt(6) , gge(9) , ggu(9) , gi(36) , glay(9) , glayt(9) , gpnorm(4,4) ,   &
      & gprop(25) , gt(9) , gtemps(6) , jacob(9) , mintr , mominr , offset , pa , phi(9) , pt(24) , ptg(24) , ptint(2) , reali , s ,&
      & s2 , shp(4) , stiff(36) , talfab(3) , talfam(3) , tbar , tbg(9) , tbm(9) , teb(9) , tem(9) , tempel , teu(9) , tgrad ,      &
      & tgrid(4,4)
...
!
               DO ir = 1 , 3
                  ...
                  IF ( lamopt/=mem .AND. lamopt/=symmem ) ftherm(ir+3) = ftherm(ir+3) - galpha(ir)*deltat*((zk**2)-(zk1**2))/2.0
               ENDDO
!
...
!
                  DO ir = 1 , 3
                     ftherm(ir) = ftherm(ir) + galpha(ir)*deltat*(zk-zk1)
                     IF ( lamopt/=symmem ) ftherm(ir+3) = ftherm(ir+3) - galpha(ir)*deltat*((zk1**2)-(zk**2))/2.0
                  ENDDO
               ENDIF
!

...
SUBROUTINE ta1b
   ...
   INTEGER bar , blk(2) , buf(50) , buf1 , buf2 , buf3 , buf4 , cbar , elemid , file , flag , gpsav(34) , i , id , idcntr , idptr , &
         ...
         & izero , j , jscalr , jtemp , k , k1 , khi , kk , klo , kscalr , ktwo24 , kx , l , last , length , lj , ll , llx ,        &
         & locbgp , locgpc , locsil , loctmp , locx , lq , lstprp , lx , m , maxel , mm , n , n2 , n21 , nam(2) , name , nbgp ,     &
         & ndx1 , ndx2 , neq1 , neq2 , nid , noect , nogpct , npvt , ntemp , ntmp , ntmpid , number
   REAL bufr(50) , tgrid(33)
...
!
      IF ( Elem(i)/=plot ) THEN
         ...
         DO
            CALL read(*3200,*100,Ect,buf,m,0,flag)
            DO l = lx , mm
               k = buf(l)
...
!WKBR CALL GOPEN (EQEXIN,EQEXIN,Z(BUF1),RDREW)
            ...
            CALL read(*3100,*120,Eqexin,Z(neq1),buf4,1,neq2)
         ELSE
            GOTO 140
         ENDIF
...
!
            ...
            DO i = 1 , Jlast , Incr
               IF ( Elem(i)/=plot ) THEN
                  ...
                  mm = lx + Elem(i+9) - 1
                  DO
                     CALL read(*3200,*185,Ect,buf,m,0,flag)
                     DO l = lx , mm
                        k = buf(l)
...
!
 ...
 200  IF ( Elem(ielem+3)==plotel .OR. Elem(ielem+3)==react ) GOTO 400
   ...
   last = buf3 - m
 300  DO WHILE ( l<=last )
      CALL read(*3200,*400,Ect,Z(l),m,0,flag)
      l = l + m
      ll = ll + m
   ENDDO
   GOTO 500
...
!
 500  CALL open(*3100,scri,Z(buf2),Rdrew)
   CALL open(*3100,scro,Z(buf3),Wrtrew)
 600  DO
      CALL read(*800,*700,scri,buf,1,0,flag)
      IF ( buf(1)<0 ) THEN
         CALL read(*3200,*3300,scri,buf(2),1,0,flag)
         k = buf(2)/two24
         ktwo24 = k*two24
         idptr = buf(2) - ktwo24
         kk = Z(k) + idptr - Z(k+1)
      ...
      ELSEIF ( buf(1)==0 ) THEN
         ...
         CALL fwdrec(*3200,scri)
      ELSE
         CALL read(*3200,*3300,scri,buf(2),buf(1),0,flag)
      ...
      DO ii = 1 , n2 , 2
         ...
         ELSE
            ...
            DO
               ...
               CALL read(*3200,*820,Ept,Z(l),m,0,flag)
               l = l + m
            ENDDO
         ENDIF
 820     n = l - Z(ii)
...
!     IF DEFINED
!
         CALL open(*3100,scro,Z(buf1),Rdrew)
         CALL open(*3100,scri,Z(buf2),Wrtrew)
         oufile = scri
         GOTO 1500
      ENDIF
   ELSE
...
!
         ...
         Eorflg = .FALSE.
         Endid = .TRUE.
         CALL read(*3200,*3300,Gptt,iset,1,0,flag)
         IF ( iset/=Tempid ) THEN
...
!
 1200 CALL read(*3200,*1400,infile,buf(1),1,0,flag)
   IF ( buf(1)<0 ) GOTO 3500
   IF ( buf(1)==0 ) THEN
!
...
!
      ...
      ll = ll + 1
      CALL fwdrec(*3300,infile)
      GOTO 1100
   ELSE
      CALL read(*3200,*3300,infile,buf(2),buf(1),0,flag)
      ik = buf(2)/two24
      ii = ((ik-1)/2)*Incr + 1
      lx = Elem(ii+12) + 1
      m = Elem(ii+8)
...
!
         ...
         CALL fwdrec(*3200,scro)
      ELSE
         CALL read(*3200,*3300,scro,buf(2),buf(1),0,flag)
         ik = buf(2)/two24
         ii = ((ik-1)/2)*Incr + 1
         m = Elem(ii+8)
         nx = buf(1) + 1
...
!
      ...
      GOTO 1900
   ENDIF
 2200 CALL write(oufile,Z(kx+1),m-1,0)
   GOTO ret
!
...
!
 2300 nbgp = 0
   locbgp = l
   IF ( kscalr/=0 ) THEN
      CALL open(*3100,Bgpdt,Z(buf1),Rdrew)
      CALL fwdrec(*3200,Bgpdt)
      nbgp = 4*Nsil
      CALL read(*3200,*3300,Bgpdt,Z(locbgp),nbgp,1,flag)
      CALL close(Bgpdt,Clsrew)
   ENDIF
   l = l + nbgp
   CALL open(*3100,Sil,Z(buf1),Rdrew)
   CALL fwdrec(*3200,Sil)
   locsil = locbgp + nbgp
   CALL read(*3200,*3300,Sil,Z(locsil),Nsil,1,flag)
   ...
   Itemp = Tempid
   Iback = 0
   IF ( Tempid==0 ) GOTO 2600
   file = Gptt
   CALL open(*3100,Gptt,Z(buf4),Rdrew)
   CALL read(*3200,*2400,Gptt,Z(loctmp),buf3-loctmp,0,nid)
   CALL mesage(-8,0,nam)
 2400 itmpid = loctmp + 2
   ntmpid = loctmp + nid - 3
   DO ijk = itmpid , ntmpid , 3
      IF ( Tempid==Z(ijk) ) GOTO 2500
   ENDDO
   buf(1) = Tempid
   buf(2) = 0
   n = 44
...
!
 2700 CALL open(*3100,infile,Z(buf1),Rdrew)
   CALL open(*3100,Ecpt,Z(buf2),Wrtrew)
   CALL fname(Ecpt,buf)
   CALL write(Ecpt,buf,2,1)
   nogpct = 0
   CALL open(*2800,Gpct,Z(buf3),Wrtrew)
   nogpct = 1
   CALL fname(Gpct,buf)

...
!
   ...
   Ifail = 0
   kstep = 1
 100  CALL alg29(vwup,r,xx2,Nstrms)
   DO j = 1 , Nstrms
 ...
 200  j2 = j1 + jstep
   ...
   DO
      ...
      ENDIF
   ENDDO
 300  CALL alg29(hsup,r,xx2,Nstrms)
   DO j = 1 , Nstrms
   ...
   DO
      ...
      IF ( q2>=Hmin ) THEN
         ...
         ELSEIF ( j2==1 ) THEN
            ...
            xx1(1) = 0.0
            DO j = 1 , itub
               xx1(j+1) = xx1(j) + (r(j+1)**2-r(j)**2)*(vzdn(j+1)+vzdn(j))*alg5((hsdn(j)+hsdn(j+1))*0.5,(sdn(j)+sdn(j+1))*0.5)
            ENDDO
            q1 = xx1(Nstrms)

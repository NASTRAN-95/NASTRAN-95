...
SUBROUTINE cf1fbs(Tpose,Xout,Iobuf)
   IMPLICIT NONE
   REAL Aadum(117) , Cdp , Da(2) , Eofnrw , Rd , Rdp , Rdrew , Rew , Rsp , Wrt , Wrtrew , Xa(2) , Xcd01(4) , Xcd02(9) , Xcd03(6)
   INTEGER Csp , Eol , Ii , Iscr6 , Ksystm , Mcblt(7) , Mcbut(7) , Norew , Nout , Nswp
   LOGICAL Qpr , Symmet
...
!*******
      CALL gopen(Mcblt(1),Iobuf(1),Rdrew)
      j = 1
      CALL intpk(*400,Mcblt(1),0,Csp,0)
   ENDIF
 100  DO WHILE ( Eol==0 )
      CALL zntpki
...
99006 FORMAT (1H ,15(1X,I7))
      ...
      j = Nswp
   ENDIF
 500  CALL intpk(*1000,Mcbut(1),0,Csp,0)
   IF ( Eol/=0 ) GOTO 1000
 600  CALL zntpki
   IF ( Qpr ) WRITE (Nout,99012) Da , Ii , Eol , j
   i = Nswp - Ii + 1

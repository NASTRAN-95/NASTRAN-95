...
SUBROUTINE draw(Gplst,X,U,S,Disp,Stereo,Opcor,Buf1)
   ...
   INTEGER Color , Daxis(3) , Elset , Icntvl , Model , Ncntr(51) , Ngp , Ngpset , Nopens , Pcon , Pedge , Plabel , Pltnum , Porig , &
         & Ppen , Prject , Pset , Pshape , Psymbl(2) , Psymm(6) , Pvectr
   COMMON /blank / Ngp , Skp11 , Pltnum , Ngpset , Skp12 , Skp21 , Elset
   COMMON /drwdat/ Pset , Plabel , Porig , Ppen , Pshape , Psymbl , Psymm , Pvectr , Pcon , Pedge
   COMMON /pltdat/ Model , Ploter , Reg , Axymax
...
!
   ...
   IF ( ishape==0 ) CALL shape(*400,Gplst,X,0,pen,0,iopt,iptl,S(sucor,1),Opcor)
   IF ( Pedge>=2 ) THEN
   ...
   ENDIF
   IF ( Pcon/=0 ) THEN
      IF ( .NOT.Disp .OR. Pshape<3 ) GOTO 100
      later = Pcon
      Pcon = 0
   ENDIF
   IF ( Pedge==0 .OR. Pedge>=2 ) GOTO 200
 100  iopt = -1
   ...
   IF ( Pedge/=1 .AND. Color<0 ) THEN
      ...
      CALL shape(*400,Gplst,X,0,1,0,iopt,iptl,S(sucor,1),Opcor)
   ENDIF
 200  Pcon = max0(Pcon,later)
   IF ( Ppen>31 ) CALL shape(*400,Gplst,X,0,1,0,iopt,iptl,S(sucor,1),Opcor)
   IF ( Pshape==1 ) Pcon = 0
   IF ( Psymbl(1)/=0 ) THEN
      IF ( Disp ) THEN
         sym(1) = 2
   ...
   IF ( Plabel>=0 ) THEN
      ...
      ENDIF
   ENDIF
 300  IF ( .NOT.(.NOT.Disp .OR. Maxdef==0.0 .OR. Defmax==0.0) ) THEN
      IF ( Pedge/=3 ) THEN
...
!
            IF ( Pvectr>=0 ) THEN
               ...
               IF ( Pedge==0 ) CALL shape(*400,Gplst,X,S,pen,1,iopt,iptl,S(sucor,1),Opcor)
               IF ( Pedge>=2 ) THEN
            ...
            IF ( Ppen>31 ) CALL shape(*400,gpl,X,0,1,0,iopt,iptl,S(sucor,1),Opcor)
            IF ( Psymbl(1)/=0 ) THEN
...
!
 400  IF ( Pedge/=1 ) CALL pcoord(pen)
END SUBROUTINE draw
...
SUBROUTINE alg13(Ibl,Ys,Yp,Xs,Xp,Ysemi,Xsemi,Log1,Log2,N,Iprint,Beta1,Beta2,P,Q,Yzero,T,Yone,Xdel,Ydel,Z,Axialc,Lnct,Ifcord,Sq,Sb,  &
   ...
   REAL a , a1 , a2 , a4 , aform , am(80) , ang , area , area2 , areas , at , ax , b , b1 , b2 , beta3 , bform , bta1 , bta2 , c ,  &
      ...
      & ixyn , iy , iyd , iyn , oa , oa48 , phi(11) , phi1 , phi2 , phip , phipp , phis , phiss , phix , pi , r1 , r2 , r3 , rab ,  &
      & rdius , rdius1
   REAL f1 , f2 , f3 , f4 , f5 , f6 , f7 , f8
   INTEGER i1 , i2 , i3 , ik , isec1 , isec2 , j , jj , jm1 , k1 , k11 , kj , kl , km , mk , name(2) , nasnum , nnn , nq
   REAL rdius2 , rle , rp , rs , rx1 , rx2 , rxbar , s(80) , s0 , s2 , s23 , s3 , sbar , sigdum , sigmao , sn , snadum(10) ,        &
      & sndum1 , sndum2 , ss2 , ssdum , sspls , sspltm(45) , ssurf , stager , t2 , thick(45) , thick2(80) , tm , tprim2 , ts1 , x0 ,&
      & x1 , x11 , x12 , xab , xbar , xbarb , xbarc , xbars , xbc , xc , xd , xdum , xint , xk2 , xm(81) , xml , xmlc , xmlcs ,     &
...
99003 FORMAT (2X,/5X,4HLINE,I3,4H  P=,F7.4,4H  Q=,F7.4,8H  BETA1=,F7.3,8H  BETA2=,F7.3,8H  YZERO=,F7.5,6H  T/C=,F7.5,7H  YONE=,F7.5,&
             &4H  Z=,F7.4,6H  AXC=,F7.3)
   ENDIF
   IF ( Isecn==1 ) THEN
      nq = 1
      Sb = Beta1 + Sb
   ...
   ENDIF
 100  chord = xmlc/(1.0-Yzero+xmlc*(Yzero+abs(Yone*sin(Beta2/c1))))
   fcslmn = 1.0 - chord*(Yzero+abs(Yone*sin(Beta2/c1)))
 200  IF ( Ifcord==1 ) Axialc = Axialc/chord
   ...
   IF ( Isecn/=2 ) THEN
      ...
      IF ( Isecn/=3 ) THEN
         ...
         DO j = 2 , N
            DO jj = 1 , 11
               GOTO isec1
 205           phi(jj) = sqrt(1.0+(oa/12.0*(xx-h)**3+xk2*2.0*xx+b)**2)
               GOTO 215
 210           xd = xx - Sq
               IF ( xd>0.0 ) THEN
 ...
 215           xx = xx + delx
   ...
   DO j = 1 , N
      ...
      ELSE
         ...
         ENDIF
      ENDIF
 250  ym(j) = oa48*(xm(j)-h)**4 + xk2*xm(j)**2 + b*xm(j) + c
...
99004 FORMAT (/16X,100HNORMALISED RESULTS - ALL THE FOLLOWING REFER TO ABLADE HAVING A MERIDIONAL CHORD PROJECTION OF UNITY,/16X,   &
            & 100(1H*),//20X,11HBLADE CHORD,4X,1H=,F7.4,//20X,16HSTAGGER ANGLE  =,F7.3,//20X,16HCAMBER ANGLE   =,F7.3,//20X,        &
             &16HSECTION AREA   =,F7.5,//20X,45HLOCATION OF CENTROID RELATIVE TO LEADING EDGE,//30X,6HXBAR =,F8.5,/30X,6HYBAR =,    &
            & F8.5,//20X,37HSECOND MOMENTS OF AREA ABOUT CENTROID,//30X,6HIX   =,F8.5,/30X,6HIY   =,F8.5,/30X,6HIXY  =,F8.5,//20X,  &
             &58HANGLE OF INCLINATION OF (ONE) PRINCIPAL AXIS TO  X  AXIS =,F7.3,//20X,                                             &
             &47HPRINCIPAL SECOND MOMENTS OF AREA ABOUT CENTROID,//30X,6HIPX  =,F7.5,6X,3H(AT,F7.3,15H WITH  X  AXIS),/30X,6HIPY  =,&
            & F7.5,6X,3H(AT,F7.3,15H WITH  Y  AXIS),//)
      WRITE (Log2,99020)
      DO j = 1 , N
         IF ( Lnct==60 ) THEN
            WRITE (Log2,99019)
...
99006 FORMAT (1H1,45X,33HNORMALISED PLOT OF SECTION NUMBER,I3,/2X)
   ...
   IF ( Isplit/=0 ) THEN
      ...
      IF ( Iprint>=2 ) GOTO 700
      IF ( Isplit==2 ) THEN
         WRITE (Log2,99007) chord , rle , xc , yc , xtc , ytc , xbars , ybars , areas
99007    FORMAT (1H1,31X,69HDIMENSIONAL RESULTS - ALL RESULTS REFER TO A BLADE OF SPECIFIED CHORD,/32X,69(1H*),//20X,11HBLADE CHORD,&
               & 4X,1H=,1P,E12.5,//20X,10HEND RADIUS,5X,1H=,1P,E12.5,8X,14HCENTERED AT X=,1P,E12.5,3H Y=,1P,E13.5,/64X,6HAND X=,1P, &
               & E12.5,3H Y=,1P,E13.5,/20X,26HLOCATION OF CENTROID AT X=,1P,E12.5,7H AND Y=,1P,E12.5,//20X,16HSECTION AREA   =,1P,  &
               & E12.5,//2X)
      ELSE
         WRITE (Log2,99008) chord , rle , xc , yc , xbars , ybars , areas
99008    FORMAT (1H1,31X,69HDIMENSIONAL RESULTS - ALL RESULTS REFER TO A BLADE OF SPECIFIED CHORD,/32X,69(1H*),//20X,11HBLADE CHORD,&
               & 4X,1H=,1P,E12.5,//20X,10HEND RADIUS,5X,1H=,1P,E12.5,8X,14HCENTERED AT X=,1P,E12.5,3H Y=,1P,E13.5,//20X,            &
                &26HLOCATION OF CENTROID AT X=,1P,E12.5,7H AND Y=,1P,E12.5,//20X,16HSECTION AREA   =,1P,E12.5,//2X)
      ENDIF
   ELSE
      chord = chord*Axialc
      Ccord(Ibl) = chord
...
99009    FORMAT (1H1,31X,69HDIMENSIONAL RESULTS - ALL RESULTS REFER TO A BLADE OF SPECIFIED CHORD,/32X,69(1H*),//20X,11HBLADE CHORD,&
               & 4X,1H=,1P,E12.5,//20X,9HEND RADII,6X,1H=,1P,E12.5,8X,14HCENTERED AT X=,1P,E13.5,3H Y=,1P,E13.5,/64X,6HAND X=,1P,   &
               & E13.5,3H Y=,1P,E13.5,/20X,16HSECTION AREA   =,1P,E12.5,//20X,37HSECOND MOMENTS OF AREA ABOUT CENTROID,//30X,       &
                &6HIX   =,1P,E12.5,/30X,6HIY   =,1P,E12.5,/30X,6HIXY  =,1P,E12.5,//20X,                                             &
                &47HPRINCIPAL SECOND MOMENTS OF AREA ABOUT CENTROID,//30X,6HIPX  =,1P,E12.5,5H  (AT,0P,F7.3,15H WITH  X  AXIS),/30X,&
                &6HIPY  =,1P,E12.5,5H  (AT,0P,F7.3,15H WITH  Y  AXIS),//)
      ELSE
         WRITE (Log2,99010) chord , rle , xc , yc , area , ix , iy , ixy , ipx , ang , ipy , ang
99010    FORMAT (1H1,31X,69HDIMENSIONAL RESULTS - ALL RESULTS REFER TO A BLADE OF SPECIFIED CHORD,/32X,69(1H*),//20X,11HBLADE CHORD,&
               & 4X,1H=,1P,E12.5,//20X,10HL.E.RADIUS,5X,1H=,1P,E12.5,8X,14HCENTERED AT X=,1P,E13.5,3H Y=,1P,E13.5,//20X,            &
                &16HSECTION AREA   =,1P,E12.5,//20X,37HSECOND MOMENTS OF AREA ABOUT CENTROID,//30X,6HIX   =,1P,E12.5,/30X,6HIY   =, &
               & 1P,E12.5,/30X,6HIXY  =,1P,E12.5,//20X,47HPRINCIPAL SECOND MOMENTS OF AREA ABOUT CENTROID,//30X,6HIPX  =,1P,E12.5,  &
                &5H  (AT,0P,F7.3,15H WITH  X  AXIS),/30X,6HIPY  =,1P,E12.5,5H  (AT,0P,F7.3,15H WITH  Y  AXIS),//)
      ENDIF
   ENDIF
   WRITE (Log2,99022)
   WRITE (Log2,99023)
 ...
 700  DO j = 1 , N
      ...
      IF ( Iprint<2 ) THEN
         IF ( (j/2)*2==j ) THEN
            ...
            Lnct = Lnct + 1
            jm1 = j - 1
            WRITE (Log2,99011) jm1 , Xs(Ibl,jm1) , Ys(Ibl,jm1) , Xp(Ibl,jm1) , Yp(Ibl,jm1) , j , Xs(Ibl,j) , Ys(Ibl,j) , Xp(Ibl,j) ,&
                             & Yp(Ibl,j)
...
99014       FORMAT (48X,I5,1P,E17.5,1P,E14.5)
      ...
      GOTO 900
   ENDIF
 800  phiss = phis - abs((Beta1-Beta2)/c2)
   ...
   IF ( Iprint<2 ) THEN
      WRITE (Log2,99015)
99015 FORMAT (//39X,44HPOINTS DESCRIBING LEADING AND TRAILING EDGES,/25X,12HLEADING EDGE,22X,13HTRAILING EDGE,/2X,9HPOINT NO.,4X,8X,&
             &1HX,14X,1HY,12X,8X,1HX,14X,1HY,/2X)
...
99016 FORMAT (6X,I2,7X,1P,E17.5,1P,E14.5,2X,1P,E17.5,1P,E14.5)
 ...
 900  ssurf = Axialc
   ...
   DO ik = 2 , 100
      IF ( xbar==Dx(ik) ) GOTO 1000
      IF ( xbar>Dx(ik-1) .AND. xbar<Dx(ik) ) GOTO 1100
   ENDDO
   WRITE (Log2,99017)
99017 FORMAT (1H1,23H XBAR CANNOT BE LOCATED)
 1000 Sigma(ik) = sigmao
   kl = ik + 1
   GOTO 1200
 1100 kl = ik
   Sigma(ik-1) = sigmao

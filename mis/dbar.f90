...
SUBROUTINE dbar
   ...
   INTEGER Icstm , Iecpt(100) , Ielid , Igsub0 , Imatid , Ipinfl(2) , Isilno(2) , Iz(1) , Matflg , Matidc , Mcsida , Mcsidb ,       &
         & Ncstm , Nogo , Npvt , Nu
   COMMON /ds1aaa/ Npvt , Icstm , Ncstm , Dumcl , Nogo
   COMMON /ds1adp/ Ke , Kep , Dela , Delb
   COMMON /ds1aet/ Ielid , Isilno , Smallv , Igsub0 , Ipinfl , Za , Zb , Gef , Imatid , A , Dummy1 , Dummy2 , I1 , I2 , Dummy3 ,    &
                 & Fj , Fmu , K1 , K2 , Dum2 , Mcsida , Gpa , Mcsidb , Gpb , Tempel , Eldef , Temper , Uas , Ubs , Dum3
   ...
   DOUBLE PRECISION alpha , da , dfj , dpveca(6) , dpvecb(6) , e , ei1 , ei2 , fl , fll , fx , gak1 , gak2 , kc(12,12) , kd(144) ,  &
                  ...
                  & sk3 , sk4 , smalv0(6) , ta(18) , tb(9) , term1 , term10 , term11 , term2 , term3 , term4 , term5 , term6 ,      &
                  & term7 , term8 , term9 , tsub0 , ua(6) , ub(6) , veci(3) , vecj(3) , veck(3) , vy , vz
                  ...
                  IF ( kes(jj)/=0.0D0 ) THEN
                     DO i = 1 , 12
                        ...
                        DO l1 = 1 , 12
                           il = 12*(i-1) + l1
                           lj = 12*(l1-1) + ipin(j)
                           kdp(il) = kd(il) - kes(lj)*kd(ji)/kes(jj) - kes(ji)*kd(lj)/kes(jj) + kes(lj)*kes(ji)*kd(jj)/kes(jj)**2
                        ENDDO
                     ENDDO
                     DO kk = 1 , 144
                        kd(kk) = kdp(kk)

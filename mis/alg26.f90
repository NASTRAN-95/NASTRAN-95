...
99006    FORMAT (5X,4HPASS,I3,9H  STATION,I3,11H  ITERATION,I3,12H  STREAMLINE,I3,6H  LOOP,I3,33H  MERIDIONAL VELOCITY ABOVE LIMIT, &
...
99008       FORMAT (5X,4HPASS,I3,9H  STATION,I3,11H  ITERATION,I3,12H  STREAMLINE,I3,38H  MERIDIONAL VELOCITY UNCONVERGED  VM=,     &
 ...
 500  Delw(1) = 0.0
   ...
   IF ( dv>0.1*Vv(Imid) ) dv = 0.1*Vv(Imid)
 600  IF ( .NOT.(Ipass==1 .OR. (I/=1 .AND. Nwork(I)<=4)) ) THEN
...
99009       FORMAT (5X,4HPASS,I3,9H  STATION,I3,11H  ITERATION,I3,12H  STREAMLINE,I3,                                               &
   ...
   IF ( abs(w/Flow(Icase)-1.0)>x1 ) GOTO 800
   DO j = k , Nstrms
      IF ( abs(Vv(j)/vvold(j)-1.0)>x1 ) GOTO 800
   ENDDO
   GOTO 900
 700  Ifail = 1
   IF ( Ipass>Nforce ) THEN
...
99011 FORMAT (5X,4HPASS,I3,9H  STATION,I3,49H  MOMENTUM AND/OR CONTINUITY UNCONVERGED W/WSPEC=,F8.5,16H VM/VM(OLD) HUB=,F8.5,       &
   ...
   ENDIF
 900  IF ( Ifail/=0 .AND. Ifailo==0 ) Ifailo = I
   DO j = 1 , Nstrms
      Vm(j,I) = Vv(j)
   ENDDO
   IF ( I/=1 ) THEN

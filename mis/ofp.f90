...
!
   ...
   CHARACTER*23 Ufm
   CHARACTER*29 Uim
   COMMON /blank / Icard , Option
   COMMON /ofp1id/ Idm
   COMMON /ofpbd1/ D
   COMMON /ofpbd5/ Esingl , E
   ...
   INTEGER a4 , b(23,4) , bk1 , ceigen , center , comma , cparen , device , e156 , e236 , e9pt1 , eend , f156 , f174 , f236 , file ,&
         ...
         & ih0 , iharm , ihd , iheat , imag(5) , incr , iopt , iout(100) , isave(20) , istar , itemp , itype , ix , j , jj , k ,    &
         & k1 , k2 , kk , lastid , maxn , nadd , nharm , nlines , nmult , nwds , nwdsav , oldhrm , oparen , pe , pf
...
!
 200  ifile = ifile + 1
   IF ( onefil .AND. ifile>1 ) GOTO 2900
   file = filex(ifile)
   IF ( onefil ) file = Ifile1
   CALL open(*2800,file,Buff(1),0)
   from = 55
   CALL fwdrec(*2600,file)
 300  DO
      CALL read(*2700,*2700,file,Id(1),50,0,flag)
      CALL read(*2700,*2700,file,Head(1),96,1,flag)
      axic = .FALSE.
      Temper = .FALSE.
      dummy = .FALSE.
      gpst = .FALSE.
...
!
         from = 340
         IF ( j>1 ) GOTO 2600
         CALL ofpgpw(*2600,file,dout,from)
         CYCLE
...
!WKBR NCL93012 3/94     1    ID(3).NE.19) GO TO 2020
         IF ( Id(3)/=6 .AND. Id(3)/=17 .AND. Id(3)/=18 .AND. Id(3)/=19 .AND. Id(3)/=64 .AND. Id(3)/=83 ) GOTO 2600
         from = 415
         IF ( icurv>2 ) GOTO 2600
         strain = .TRUE.
         elemen = .TRUE.
         iopt = 2
         nadd = 0
...
!     DISPLACEMENT VECTOR
!
 320     IF ( j==3 .AND. iapp==trans ) nadd = 7
...
!
 350  IF ( j==3 .AND. iapp==trans ) nadd = 7
!WKBI 11/93 SPR93007
      pack = .TRUE.
      GOTO 450
!
...
!
 400  from = 240
      IF ( Id(3)<1 .OR. Id(3)>100 ) GOTO 2600
...
!
 ...
 600  IF ( eor ) GOTO 300
   from = 900
   CALL read(*2600,*700,file,iout(1),nwds,0,flag)
   GOTO 800
 700  IF ( flag==0 ) GOTO 300
   IF ( flag==0 ) GOTO 300
   nwds = flag
   nwdsav = nwds
   eor = .TRUE.
 800  i1 = 0
   IF ( .NOT.(axic) ) THEN
      ...
      IF ( .NOT.(ese .OR. gpfb) ) THEN
         IF ( .NOT.pack .AND. .NOT.elemen ) GOTO 1000
      ENDIF
   ENDIF
   IF ( sort==2 ) GOTO 900
   incr = Id(10)
...
!
      ...
      ENDIF
   ENDIF
   IF ( device/=1 .AND. device/=5 ) GOTO 600
 1000 IF ( .NOT.pack .OR. axic ) GOTO 1200
   IF ( fluid .AND. sort==1 .AND. iout(1)>=500000 ) GOTO 2100
 1100 IF ( iout(2)/=gse(1) ) THEN
!
...
!
            IF ( k==430 .AND. Id(9)==3 .AND. iapp==freq .AND. fmt(ifmt-4)==phase ) GOTO 1600
         ELSE
            iout(i) = iblank
            GOTO 1600
         ENDIF
...
!
 1700 Line = Line + nlines
   IF ( eor ) GOTO 300
   IF ( axic ) GOTO 600
   IF ( .NOT.pack .OR. iout(2)==gse(1) .OR. i1==9 ) GOTO 600
!
...
!
   DO i = 1 , nwds
      iout(i) = isave(i)
   ENDDO
   IF ( .NOT.fluid ) GOTO 1100
   IF ( iout(1)<500000 ) GOTO 1100
   GOTO 2100
 1800 IF ( i==6 ) GOTO 1900
   DO
      from = 1740
      CALL read(*2600,*2000,file,isave(1),nwds,0,flag)
      igse = isave(2)
...
!     DECODE THE HARMONIC
!
 2200 itemp = mod(iout(1),500000)
   nharm = (iout(1)-itemp)/500000
   iout(1) = itemp
   IF ( oldhrm==-1 ) oldhrm = nharm
   IF ( nharm/=oldhrm .OR. k>=5 ) GOTO 2500
   k = k + 1
 2300 real(2*k-1) = iout(1)
   real(2*k) = iout(3)
   imag(k) = iout(9)
   DO
      from = 1810
      CALL read(*2600,*2400,file,iout(1),nwds,0,flag)
!
!     PUNCH PROCESSING
!
      itemp = iout(1)/10
...
!     CLOSE FILE UP
!
 2700 CALL close(file,1)
 2800 IF ( ifile/=6 ) GOTO 200
!

...
99005    FORMAT (4X,3HNO.,7X,5HORDER,30X,9HFREQUENCY,11X,9HFREQUENCY,12X,4HMASS,14X,9HSTIFFNESS,/)
         IF ( Id22==-999 ) GOTO 400
         IF ( Id22<0 ) THEN
            IF ( M/=8 .AND. M/=12 ) GOTO 400
            WRITE (L,99182)
            WRITE (L,99006)
99006       FORMAT (1H+,42X,'NASTRAN WARNING MESSAGE 3309, ALL LOWER EIGEN','VALUES',6X,1H*,/37X,1H*,5X,'NOT NECESSARY FOUND.',37X, &
                   &1H*)
            GOTO 100
         ELSEIF ( Id22==0 ) THEN
            IF ( .NOT.((M>=3 .AND. M<=7) .OR. M==10 .OR. M==13) ) GOTO 400
            WRITE (L,99182)
            WRITE (L,99007)
...
99029    FORMAT (4X,3HID.,11X,8HNORMAL-X,7X,8HNORMAL-Y,7X,8HSHEAR-XY,6X,12HSTRESS ANGLE,9X,5HMAJOR,10X,5HMINOR,10X,5HSHEAR)
      ...
      ELSEIF ( Line==27 ) THEN
         WRITE (L,99030)
99030    FORMAT (2X,7HELEMENT,6X,5HFIBRE,15X,'STRESSES IN ELEMENT COORD ','SYSTEM',13X,'PRINCIPAL STRESSES (ZERO SHEAR)',12X,3HMAX)
         GOTO 400
      ELSEIF ( Line==28 ) THEN
         WRITE (L,99031)
99031    FORMAT (4X,3HID.,7X,8HDISTANCE,11X,8HNORMAL-X,7X,8HNORMAL-Y,6X,8HSHEAR-XY,7X,5HANGLE,9X,5HMAJOR,11X,5HMINOR,10X,5HSHEAR)
         GOTO 400
      ELSEIF ( Line==29 ) THEN
         WRITE (L,99032)
...
99034    FORMAT (8X,5HPOINT,10X,11HSINGULARITY,18X,'LIST OF COORDINATE ','COMBINATIONS THAT WILL REMOVE SINGULARITY')
      ...
      ELSEIF ( Line==32 ) THEN
         WRITE (L,99035)
99035    FORMAT (9X,3HID.,3X,4HTYPE,7X,5HORDER,7X,21HSTRONGEST COMBINATION,15X,18HWEAKER COMBINATION,17X,19HWEAKEST COMBINATION)
         GOTO 400
      ELSEIF ( Line==33 ) THEN
         WRITE (L,99036)
...
99070    FORMAT (18X,'S T R E S S E S   I N   B E N D I N G   Q U A D R I',' L A T E R A L S',13X,'( C Q D P L T )')
      ...
      ELSEIF ( Line==68 ) THEN
         WRITE (L,99071)
99071    FORMAT (18X,'S T R E S S E S   I N   G E N E R A L   Q U A D R I',' L A T E R A L   E L E M E N T S',6X,'( C Q U A D 1 )')
         GOTO 400
      ELSEIF ( Line==69 ) THEN
         WRITE (L,99072)
99072    FORMAT (18X,'S T R E S S E S   I N   G E N E R A L   Q U A D R I',' L A T E R A L   E L E M E N T S',6X,'( C Q U A D 2 )')
         GOTO 400
      ELSEIF ( Line==70 ) THEN
         WRITE (L,99073)
...
99082    FORMAT (5X,3HID ,6X,5HPOINT,17X,3H(X),21X,7H(THETA),21X,3H(Z),23X,4H(ZX))
      ...
      ELSEIF ( Line==80 ) THEN
         WRITE (L,99083)
99083    FORMAT (11X,'S T R E S S   R E S U L T A N T S   F O R   T H E  ',' T O R O I D A L   R I N G S     ( C T O R D R G )')
         GOTO 400
      ELSEIF ( Line==81 ) THEN
         WRITE (L,99084)
99084    FORMAT (5X,3HEL ,8H  STRESS,15X,17HMEMBRANE (FORCES),26X,17HFLEXURE (MOMENTS),23X,5HSHEAR)
      ...
      ELSEIF ( Line==82 ) THEN
         WRITE (L,99085)
99085    FORMAT (5X,2HID,9H    POINT,8X,10HTANGENTIAL,10X,'CIRCUMFERENTIAL',8X,10HTANGENTIAL,11X,15HCIRCUMFERENTIAL,10X,7H(FORCE))
         GOTO 400
      ELSEIF ( Line==83 ) THEN
         WRITE (L,99086)
...
99090    FORMAT (23X,'F O R C E S   F O R   T H E   T O R O I D A L   ','R I N G S     ( C T O R D R G )')
      ...
      ELSEIF ( Line==88 ) THEN
         WRITE (L,99091)
99091    FORMAT (5X,12HEL    CORNER,9X,6HRADIAL,8X,15HCIRCUMFERENTIAL,7X,5HAXIAL,13X,6HMOMENT,9X,13HDIRECT STRAIN,7X,9HCURVATURE)
         GOTO 400
      ELSEIF ( Line==89 ) THEN
         WRITE (L,99092)
...
99095    FORMAT (1H0,/1H0,39X,32HNUMBER OF EIGENVALUES EXTRACTED ,6(2H .),I10,/1H0,39X,30HNUMBER OF STARTING POINTS USED,7(2H .),   &
               & I10,/1H0,39X,30HNUMBER OF STARTING POINT MOVES,7(2H .),I10,/1H0,39X,36HNUMBER OF TRIANGULAR DECOMPOSITIONS ,4(2H .)&
               & ,I10,/1H0,39X,34HTOTAL NUMBER OF VECTOR ITERATIONS ,5(2H .),I10,//1H0,39X,22HREASON FOR TERMINATION,11(2H .),I10,  &
                &1H*,/,/1H0,39X,36HLARGEST OFF-DIAGONAL MODAL MASS TERM,4(2H .),E10.2,/1H0,77X,3(2H .),I10,/50X,9HMODE PAIR,10(2H .)&
               & ,/78X,3(2H .),I10,/1H0,39X,'NUMBER OF OFF-DIAGONAL MODAL ','MASS',/45X,23HTERMS FAILING CRITERION,8(2H .),I10)
         M = Id(17)
         IF ( M>=8 ) Nogo = 14
         IF ( Id(16)==1 ) THEN
...
99097    FORMAT (1H0,/1H0,39X,32HNUMBER OF EIGENVALUES EXTRACTED ,6(2H .),I9,/1H0,39X,                                              &
                &44HNUMBER OF PASSES THROUGH STARTING POINTS . .,I9,/1H0,39X,26HNUMBER OF CRITERIA CHANGES,9(2H .),I9,/1H0,39X,     &
                &30HNUMBER OF STARTING POINT MOVES,7(2H .),I9,/1H0,39X,36HNUMBER OF TRIANGULAR DECOMPOSITIONS ,4(2H .),I9,/1H0,39X, &
                &44HNUMBER OF FAILURES TO ITERATE TO A ROOT  . .,I9,//1H0,39X,22HREASON FOR TERMINATION,11(2H .),I9,1H*,//,1H0,39X, &
                &36HLARGEST OFF-DIAGONAL MODAL MASS TERM,4(2H .),E9.2,/1H0,77X,3(2H .),I9,/50X,9HMODE PAIR,10(2H .),/78X,3(2H .),I9,&
               & /1H0,39X,33HNUMBER OF OFF-DIAGONAL MODAL MASS,/45X,23HTERMS FAILING CRITERION,8(2H .),I9)
         M = Id(17)
         GOTO 300
      ELSEIF ( Line==94 ) THEN
         WRITE (L,99098)
99098    FORMAT (10X,14HSTARTING POINT,6X,6HLAMBDA,9X,'RADIAN FREQUENCY  ','  CYCLIC FREQUENCY    DETERMINANT',9X,'SCALE FACTOR',/)
         GOTO 400
      ELSEIF ( Line==95 ) THEN
         WRITE (L,99099)
...
99102    FORMAT (1H0,/1H0,39X,32HNUMBER OF EIGENVALUES EXTRACTED ,6(2H .),I9,/1H0,39X,                                              &
                &44HNUMBER OF PASSES THROUGH STARTING POINTS . .,I9,/1H0,39X,26HNUMBER OF CRITERIA CHANGES,9(2H .),I9,/1H0,39X,     &
                &30HNUMBER OF STARTING POINT MOVES,7(2H .),I9,/1H0,39X,36HNUMBER OF TRIANGULAR DECOMPOSITIONS ,4(2H .),I9,/1H0,39X, &
                &44HNUMBER OF FAILURES TO ITERATE TO A ROOT  . .,I9,/1H0,39X,36HNUMBER OF PREDICTIONS OUTSIDE REGION,4(2H .),I9,    &
               & /1H0,/1H0,39X,22HREASON FOR TERMINATION,11(2H .),I9,1H*)
         M = Id(18)
         GOTO 300
      ELSEIF ( Line==99 ) THEN
         IF ( Id(3)==4 ) THEN
            WRITE (L,99103) Id(11) , Id(12) , Id(18)
99103       FORMAT (1H0,/1H0,/,1H0,35X,32HNUMBER OF EIGENVALUES EXTRACTED ,9(2H .),I9,/,1H0,35X,30HNUMBER OF EIGENVALUES DESIRED ,  &
                  & 10(2H .),I9,/,1H0,35X,22HREASON FOR TERMINATION,14(2H .),I9,1H*)
            M = Id(18)
            GOTO 300
         ELSE
            WRITE (L,99104) (Id(k),k=11,16)
99104       FORMAT (1H0,/1H0,/1H0,35X,32HNUMBER OF EIGENVALUES EXTRACTED ,9(2H .),I9,/1H0,35X,30HNUMBER OF STARTING POINTS USED,    &
                   &10(2H .),I9,/1H0,35X,50HNUMBER OF STARTING POINT OR SHIFT POINT MOVES  . .,I9,/1H0,35X,                         &
                   &42HTOTAL NUMBER OF TRIANGULAR DECOMPOSITIONS ,4(2H .),I9,/1H0,35X,34HTOTAL NUMBER OF VECTOR ITERATIONS ,8(2H .),&
                  & I9,/1H0,/1H0,35X,22HREASON FOR TERMINATION,14(2H .),I9,1H*)
            M = Id(16)
            IF ( Id(17)==1 ) THEN
               IF ( M>2 ) Nogo = 14
               IF ( M==0 ) M = 10
...
99161 FORMAT (29X,'C O M P L E X   F O R C E S   I N   R O D   E L E M',' E N T S   ( C R O D )')
   ELSEIF ( local==57 ) THEN
      WRITE (L,99162)
99162 FORMAT (7X,'C O M P L E X   F O R C E S   A C T I N G   O N   ','S H E A R   P A N E L   E L E M E N T S   (C S H E A R)')
...
99163 FORMAT (16X,'C O M P L E X   F O R C E S   I N   B A S I C   B E',' N D I N G   T R I A N G L E S   ( C T R B S C )')
   ELSEIF ( local==59 ) THEN
      WRITE (L,99164)
99164 FORMAT (12X,'C O M P L E X   F O R C E S   I N   G E N E R A L  ',' T R I A N G U L A R   E L E M E N T S   ( C T R I A 1 )')
   ELSEIF ( local==60 ) THEN
      WRITE (L,99165)
99165 FORMAT (12X,'C O M P L E X   F O R C E S   I N   G E N E R A L  ',' T R I A N G U L A R   E L E M E N T S   ( C T R I A 2 )')
...
99172 FORMAT (17X,7HELEMENT,29X,5HAXIAL,41X,6HTORQUE,/19X,3HID.,31X,5HFORCE)
   ELSEIF ( local==68 ) THEN
      WRITE (L,99173)
99173 FORMAT (9H  ELEMENT,7X,5HFIBRE,37X,'- STRESSES IN ELEMENT COORDI','NATE SYSTEM -',/4X,3HID.,8X,8HDISTANCE,18X,8HNORMAL-X,26X, &
...
99180 FORMAT (37X,1H*,62X,1H*,/37X,1H*,8X,43H(THIS MESSAGE CAN BE SUPPRESSED BY DIAG 37),11X,1H*,/37X,16(4H****),/)
   ...
   Id22 = -999
   GOTO 400
 200  IF ( M>0 ) WRITE (L,99181) (rt(k,M),k=1,8) , sectn(1)
